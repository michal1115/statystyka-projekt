...
```{r}
set.seed(124)
data.random.forest <- randomForest(G3 ~ school + traveltime,data = data_classification,mtry = 6,importance = TRUE)
...
mean_cv_pred_err <- c()
for (cv_iter in 1:k){
  train <- data_classification[folds != cv_iter,]
  test <- data_classification[folds == cv_iter,]
